			...
			{featuresData.map((elt, index) => (
				<div key={index}>
					<div className="w-full h-auto mx-auto p-10 lg:min-w-[270px] lg:flex lg:flex-col lg:justify-center lg:items-center">
						<div className="w-40 h-40 mx-auto border-[#00BC77] border-[10px] rounded-full p-3	">
							<img
								src={`../assets/${elt.icon}`}
						...
						<div className="text-center mt-5 mb-2 font-bold text-xl w-4/5 mx-auto">
							{elt.titre}
						</div>
						<div className="text-center leading-5">{elt.text}</div>
					</div>
				</div>
			))}
		</div>
